    ...
    interval = setInterval(() => {
        ...
        for (let i = 0; i < pipes.length; i++) {
            if (birds.length > 0 && birds[0].x + birds[0].radius < pipes[i].x) {
                ...
                distanceToClosestTopPipe = birds[0].y - birds[0].radius - pipes[i].height;
        ...
        for (const bird of birds) {
            ...
            const input = [bird.gSpeed, distanceToClosestPipe, distanceToClosestTopPipe, distanceToClosestBotPipe];
            const jump = bird.brain.predict(input.map((e, i) => e / normalization[i]));
